...
class BST:
    def __init__(self,root):
        ...
        #canvas for node Presentation
        self.make_canvas = Canvas(self.window, width=1160, height=520, bg="#082A46", relief=RAISED, bd = 8)
        self.make_canvas.place(x=0, y=0)

        #canvas for result 
        self.result_canvas = Canvas(self.window, width=1160, height=520, bg="#D1D0CE", relief=RAISED, bd = 8)
    ...
    def heading_root_and_null_make(self):# By default window presentation
        make_heading = Label(self.make_canvas,text="BINARY SEARCH TREE",font=("new roman",20,"bold"),fg="#C41E3A",relief=GROOVE)
        make_heading.place(x=475,y=10)
        make_root = Label(self.make_canvas,text="Root",font=("new roman",17,"bold"),fg="brown")
        ...
        self.make_canvas.create_polygon(root_indicator, width=3, fill="yellow")

        self.make_null = Label(self.make_canvas,text="NULL",font=("new roman",17,"bold"),fg="#C41E3A")
        self.make_null.place(x=565,y=90+50)

    def make_instructional_buttons(self):# For make Instructional buttons
        #Insert button
        self.insert_node = Button(self.window,text="Insert",font=("new roman",20,"bold"),bg="black",fg="#00FF00",relief=RAISED,bd=8,command=lambda: self.take_input_set_up(1),state=NORMAL)
        self.insert_node.place(x=1210,y=10)

        #LabelFrame make for Traversing Buttons
        make_label_frame_for_traversing = LabelFrame(self.window,text="Traversing",width=50,bg="#D1D0CE",font=("new roman",13,"bold"),fg="green")
        make_label_frame_for_traversing.place(x=1183,y=180)

        self.pre_order_traversal  = Button(make_label_frame_for_traversing,text="Pre-order",font=("new roman",10,"bold"),bg="black",fg="#00FF00",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(1),state=NORMAL)
        self.pre_order_traversal.pack(padx=10,pady=5)

        self.in_order_traversal = Button(make_label_frame_for_traversing, text="In-order", font=("new roman", 10, "bold"),bg="black", fg="#00FF00", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(2), state=NORMAL)
        self.in_order_traversal.pack(padx=10, pady=5)

        self.post_order_traversal = Button(make_label_frame_for_traversing, text="Post-order", font=("new roman", 10, "bold"), bg="black", fg="#00FF00", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(3), state=NORMAL)
        self.post_order_traversal.pack(padx=10, pady=5)

        
        # Delete Button
        self.make_delete = Button(self.window, text="Delete", font=("new roman", 16, "bold"), bg="black", fg="#00FF00", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(2), state=NORMAL)
    ...
    def take_input_set_up(self,indicator):# Set up for take input value for node insert and delete
        ...
        self.take_input = Label(self.window, text="Enter the number to input", fg="blue", bg="#D1D0CE", font=("new roman", 10, "bold"))
        self.take_input.place(x=1180, y=100)
        self.take_entry = Entry(self.window, fg="blue", bg="white", relief=SUNKEN, bd=6, width=5, font=("new roman", 10, "bold"))
        self.take_entry.place(x=1200, y=130)
        self.take_entry.focus()
        self.add_btn = Button(self.window, text="Add", fg="red", bg="black", relief=RAISED, bd=6, width=5, font=("new roman", 8, "bold"), state=NORMAL)
    ...
    def make_default_node_with_set_position(self):# For node make and set at right position in canvas
        try:
            ...
            self.make_node = self.make_canvas.create_oval(505, 85+50, 555, 135+50, width=2, fill="green", outline="#D1D0CE")
            self.make_label= Label(self.window, text=self.take_entry.get(), fg="yellow", bg="green", font=("new roman", 12, "bold"))
    ...
    def decision_making_about_left_side_direction(self):
        try:
            ...
            make_greater_than_label = Label(self.window, text=">", fg="green", bg="chocolate", font=("new roman", 20, "bold"))
    ...
    def decision_making_about_right_side_direction(self):
        try:
            self.right_activate = 1
            self.left_activate = 0
            make_less_than_label = Label(self.window, text="<", fg="green", bg="white", font=("new roman", 20, "bold"))
    ...
    def make_container_in_result_canvas(self):# In down canvas
        self.traversing_label = Label(self.result_canvas, bg="#D1D0CE",fg="#C41E3A",text="Traversing Result: ",font=("new roman",15,"bold"))
        self.traversing_label.place(x=480,y=10)
        distance_maintainer = 8
        for i in range(15):
            self.value_show.append(i)
            self.value_show[i] = Label(self.result_canvas,bg="#082A46",fg="yellow",text=" ",width="4",height="1",font=("new roman",15,"bold"),relief=SUNKEN,bd=5)
            self.value_show[i].place(x= distance_maintainer,y=50)
            distance_maintainer+=91

        make_status = Label(self.result_canvas,bg="#D1D0CE",fg="#C41E3A",text="Status:",font=("new roman",17,"bold"))
        make_status.place(x=580,y=100)

        self.status_note = Label(self.result_canvas,fg="#C41E3A", bg="#D1D0CE",text="Play With BST",font=("new roman",20,"bold"))
    ...
    def left_none_right_exist(self,node):
        ...
        if left_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                make_circle = self.make_canvas.create_oval(node_coord, width=3, fill="green", outline="#D1D0CE")
    ...
    def right_none_left_exist(self, node):
        ...
        if  right_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="yellow", bg="green", font=("new roman", 12, "bold"))
                label_make.place(x=node_coord[0] + 14, y=node_coord[1] + 14)
    ...
    def color_indicator_to_delete(self,node,temp):# Color indicating to deleted and replaced node
        ...
        self.make_canvas.itemconfig(temp[0], fill="green", outline="#D1D0CE")
        temp[1].config(fg="yellow", bg="green")
        self.make_canvas.itemconfig(node[0], fill="green", outline="#D1D0CE")
